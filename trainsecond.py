...
def get_weight(in_out_shape):
    # var = tf.Variable(tf.random_normal(in_out_shape, stddev=0.35), dtype=tf.float32)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--smodel', '-m', required=False, help='smodel', default=None)

    args = parser.parse_args()

    modelPath = args.smodel
    modelFile = 'mine2.ckpt'
    ...
    if ckpt is None or ckpt.model_checkpoint_path is None:
        ...
        with tf.name_scope("output"):
            ...
            # 输出层
            output = tf.nn.leaky_relu(tf.add(tf.matmul(hidden4, W_output), b_output), name='OUTPUT')
    ...
    if args.datapath is None:
        trainingDataFile = 'training2.npz'
    else:
    ...
    if os.path.exists(trainingDataFile):
        ...
        # tempCount = 1
        # for tempNextData in trainingNextData:
        #     if tempNextData[0] == 1:
        #         tempNextData[0] = 0.
        #         tempNextData[1] = 1.
        #
        ...
        # trainingNextData = trainingNextData.astype(float)

        # np.savez(trainingDataFile, aroundData=trainingUserData, predictData=trainingNextData)

        allCount = len(trainingUserData)
        testCount = int(allCount/20)
        ...
        train_logs_path = "./logs2/train"
        test_logs_path = "./logs2/test"
